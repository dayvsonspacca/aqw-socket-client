...
final class PlayerDetectedEventTest extends TestCase
{
    ...
    {
        $event = PlayerDetectedEvent::from(DelimitedMessage::fromString(
            '%xt%uotls%-1%xionnee%mvts:-1,px:500,py:375,strPad:Spawn,bResting:false,mvtd:0,tx:0,ty:0,strFrame:Enter%',
        ));

        $this->assertInstanceOf(PlayerDetectedEvent::class, $event);

        $event = PlayerDetectedEvent::from(DelimitedMessage::fromString('%xt%exitArea%-1%1128%aejax%'));
    ...
    {
        $event = PlayerDetectedEvent::from(JsonMessage::fromString(
